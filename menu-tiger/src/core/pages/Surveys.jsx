...
import {
  FaRocket, FaUtensils, FaPlus, FaQuestion, FaChevronRight,
  FaChevronLeft, FaEdit, FaTrash, FaEllipsisV,
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      ...
      setSurveys(surveys.map(survey =>
        survey.id === editingId ? formData : survey
      ));
    } else {
      // Add new survey
      const newSurvey = {
  ...
  const renderQuestionPreview = () => {
    ...
    return (
      <div className="border border-gray-200 dark:border-gray-700 rounded-md p-4 bg-white dark:bg-gray-800">
        <h3 className="font-medium mb-2">{questionText} {isQuestionRequired && <span className="text-red-500">*</span>}</h3>
        ...
        {isAddingNew ? (
          <>
            ...
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center space-x-3">
                ...
                <div className="text-gray-900 dark:text-gray-100 text-md bg-gray-100 dark:bg-gray-700 px-4 py-3 rounded-md select-none">
                  <span>Surveys</span> <span className="text-gray-400">/</span> <span className="text-primary">
            ...
            <div className="flex border-b border-gray-300 dark:border-gray-600 mb-6">
              <button
                className={`px-4 py-2 font-medium ${currentSection === 'survey' ? 'border-b-2 border-primary text-primary' : 'text-gray-600 dark:text-gray-300'}`}
                onClick={() => setCurrentSection('survey')}
              >
                Survey
              </button>
              <button
                className={`px-4 py-2 font-medium ${currentSection === 'localize' ? 'border-b-2 border-primary text-primary' : 'text-gray-600 dark:text-gray-300'}`}
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4 bg-gray-100 dark:bg-gray-700 p-4 rounded-md">
                    ...
                    <div className="flex items-center p-2 border border-gray-300 dark:border-gray-600 rounded-md">
                      ...
                      />
                      <label htmlFor="question-required" className="text-sm font-medium text-gray-700 dark:text-gray-300">
                        Required Question
                      </label>
                    </div>
                    ...
                    <div className="flex justify-end pt-2">
                      <button
                        ...
                        className={`px-4 py-2 rounded-md text-sm font-medium ${!questionText ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-secondary hover:bg-primary text-white'}`}
                      >
                        Add Question
                      </button>
                    </div>
            ...
            {question.options.map((option, index) => (
              <div key={index} className="flex items-center justify-start">
                ...
                <div className="space-y-6 h-[calc(100vh-200px)] overflow-y-auto">
                  ...
                  {/* Welcome Note Preview */}
                  <div className="border border-gray-200 dark:border-gray-700 rounded-md p-4 bg-white dark:bg-gray-800 text-center">
                    <h2 className="text-xl font-bold mb-2">Survey: {formData.name || "[Survey Name]"}</h2>
                    <p>{formData.welcomeNote || "[Welcome note will appear here]"}</p>
                  </div>
                  ...
                  {questionText && (
                    <div className="border border-gray-200 dark:border-gray-700 rounded-md p-4 bg-white dark:bg-gray-800 text-center">
                      <h3 className="font-medium mb-4">
                        {questionText} {isQuestionRequired && <span className="text-red-500">*</span>}
                      </h3>
                      ...
                      {questionType === "yes_no" && (
                        <div className="flex justify-center space-x-4">
                          <button
                            className={`px-6 py-2 rounded-md ${yesNoAnswer === 'yes' ? 'bg-green-500 text-white' : 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200'}`}
                            onClick={() => setYesNoAnswer('yes')}
                          >
                            Yes
                          </button>
                          <button
                            className={`px-6 py-2 rounded-md ${yesNoAnswer === 'no' ? 'bg-red-500 text-white' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200'}`}
                      ...
                      {questionType === "smiley" && (
                        <div className="flex justify-center space-x-4">
                          {[
                            ...
                            <button
                              key={index}
                              className={`text-3xl hover:scale-125 transition-transform ${selectedSmiley === smiley.value ? 'scale-125' : ''}`}
                  ...
                  {formData.questions.length > 0 && (
                    <div className="space-y-4">
                      {formData.questions.map((question, qIndex) => (
                        <div key={qIndex} className="border border-gray-200 dark:border-gray-700 rounded-md p-4 bg-white dark:bg-gray-800 text-center">
                          <h3 className="font-medium mb-4">{question.text} {question.required && <span className="text-red-500">*</span>}</h3>
                              ...
                              {question.options.map((option, index) => (
                                <div key={index} className="flex items-center justify-start">
                                  <input
                                    ...
                                    className="mr-2"
                                  />
                                  <label htmlFor={`existing-option-${index}`}>{option}</label>
                                </div>
                              ))}
                            </div>
                          )}
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start">
                ...
                <div className="border border-primary rounded-lg px-4 py-1 max-w-xl flex items-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <p className="text-gray-700 dark:text-gray-300 flex-grow">
                    Gather real-time feedback with customizable surveys, gaining actionable insights to continuously improve your offerings and customer experience.
                  </p>
                </div>
              </div>
            </div>
