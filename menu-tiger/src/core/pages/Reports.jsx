  ...
  const handleSave = () => {
    ...
    if (editingId) {
      ...
      setSchedulers(schedulers.map(scheduler =>
        scheduler.id === editingId ? { ...formData, id: editingId } : scheduler
      ));
      setEditingId(null);
    } else {
      // Add new scheduler
  ...
  return (
    <div className="p-6 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-gray-100">
      ...
      <div className="bg-muted dark:bg-gray-800 px-6 pt-2 flex space-x-6 border-b border-gray-300 dark:border-gray-700 shadow-md rounded-t-lg">
        {[
          { key: "scheduler", label: "Scheduler", icon: <FaCalendarAlt className="mr-2" /> },
          { key: "newsletters", label: "Newsletters Signup", icon: <FaEnvelope className="mr-2" /> },
          { key: "feedback", label: "Feedback", icon: <FaCommentAlt className="mr-2" /> },
        ].map(({ key, label, icon }) => (
          <button
            key={key}
            onClick={() => setActiveTab(key)}
            className={`pb-3 pt-2 text-sm font-semibold flex items-center cursor-pointer ${activeTab === key
                ...
                <div className="border border-gray-200 dark:border-gray-600 shadow-sm rounded-lg overflow-hidden">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                    ...
                    <thead className="bg-gray-50 dark:bg-gray-700">
                      <tr>
                        {["Name", "Frequency", "Report To", "Actions"].map((label) => (
                          <th
                            key={label}
                            scope="col"
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-600">
                      {filteredSchedulers.length === 0 ? (
                          ...
                          <td colSpan={4} className="px-4 py-12">
                            <div className="flex flex-col items-center justify-center space-y-4">
                              ...
                              <p className="text-gray-500 dark:text-gray-400">
                                {schedulers.length === 0 ? "No reports available" : "No matching reports found"}
                              </p>
                            </div>
                          </td>
                        </tr>
                        ...
                        filteredSchedulers.map((scheduler) => (
                          <tr key={scheduler.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="text-gray-900 dark:text-gray-100 text-md bg-gray-100 dark:bg-gray-700 px-4 py-3 rounded-md select-none">
                      <span>Scheduler</span>  <span className=" text-gray-400">/</span>  <span className="text-primary">Add scheduler</span>
                    </div>
                  </div>
                  <button
        ...
        {activeTab === "newsletters" && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <button className="bg-primary text-white px-4 py-2 rounded-md text-sm hover:bg-teal-700">
                Download CSV
              </button>
              <input
                type="text"
                ...
                <thead>
                  <tr className="text-gray-700 dark:text-gray-200 border-b border-gray-300 dark:border-gray-600">
                    {["Email", "First Name", "Last Name", "Contact Number", "Date", "Image"].map((header) => (
                      <th key={header} className="px-4 py-3 font-semibold group hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  ...
                  <tr>
                    <td colSpan={2} className="text-center py-8">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        />
                        <p className="text-gray-500 dark:text-gray-400">No feedback surveys available</p>
                      </div>
                    </td>
                  </tr>
                </tbody>
